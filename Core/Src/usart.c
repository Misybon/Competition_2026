 ...
 * @brief 解析串口指令
 */
static void ReadCmd(uint32_t Size)
{
    // 清空命令
    ...
    for (uint32_t i = 0; i + 2 < Size; i++) // 遍历寻找帧头
    {
        if (g_rx_data[i] == 0xAA && g_rx_data[i + 1] == 0x55) // 解析到帧头
        {
            ...
            for (uint32_t j = i + 3; j < i + 3 + len; j++) // 解析命令
            {
                g_cmd[j - i - 3] = g_rx_data[j];
                g_rx_data[j] = '\0'; // 清除已解析的命令
            }
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef* huart)
{
    if (huart == &huart3)
    {
        ...
        HAL_UARTEx_ReceiveToIdle_DMA(&huart3, g_rx_data, BUF_SIZE); // 重新开启接收
        __HAL_DMA_DISABLE_IT(&hdma_usart3_rx, DMA_IT_HT); // 关闭半传输中断
    }
}
/* USER CODE END 1 */
